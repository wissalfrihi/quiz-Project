...
public class UpdateGroupController {
    ...
    public void initialize() {
        if (selectedGroup != null) {
            NomGroup.setText(selectedGroup.getNom());
            enseignants = userDAO.getAllUsersByRole("ENSEIGNANT");
            enseignantsToAdd = selectedGroup.getEnseignants();
            ...
            etudiantsToAdd = selectedGroup.getEtudiants();
            etudiantsEmailsField.getItems().setAll(FXCollections.observableArrayList(etudiants.stream().map(User::getEmail).toList()));
            etudiantsTodAddListView.getItems().setAll(FXCollections.observableArrayList(etudiantsToAdd.stream().map(User::getEmail).toList()));

            coursList = coursDAO.getAllCours();
            coursListToAdd = selectedGroup.getCoursList();
            coursNomField.getItems().setAll(FXCollections.observableArrayList(coursList.stream().map(Cours::getNom).toList()));
            coursTodAddListView.getItems().setAll(FXCollections.observableArrayList(coursListToAdd.stream().map(Cours::getNom).toList()));
        }
    }
